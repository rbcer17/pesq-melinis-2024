...
#This plot should include labels fixed by gemini FIG 3 FINAL REV 2026 07
#
ggplot(consolidado, aes(x = treatment_name, y = Viaveis, fill = anofactor)) + 
  geom_boxplot() + 
  # Combined viridis color palette and the drop = FALSE argument
  scale_fill_viridis_d(option = "D", drop = FALSE) +  
  scale_x_discrete(drop = FALSE) + 
  theme_classic() +
  # This will now properly display since guides(fill = "none") is removed
  labs(x = "Treatment", y = "Number of full caryopses", fill = "Year")

#END PLOT FIG 3 REV 2026 07
...
plot72 = ggplot(consol2, aes(x = coletafactor, y = pcviaveis, fill = anofactor)) + 
  geom_boxplot() + 
  scale_x_discrete(drop = FALSE) + 
  # Crucial: Keeps empty fill levels in the legend
  scale_fill_viridis_d(option = "D", drop = FALSE) + 
  ...
  theme_classic() # Note: guides(fill = "none") removed so the legend actually shows up
plot73 = ggplot(consol3, aes(x = coletafactor, y = pcviaveis, fill = anofactor)) + 
  geom_boxplot() + 
  scale_x_discrete(drop = FALSE) + 
  # Crucial: Keeps empty fill levels in the legend
  scale_fill_viridis_d(option = "D", drop = FALSE) + 
  ...
  ggtitle("Fire May") + 
  theme_classic() # Note: guides(fill = "none") removed so the legend actually shows up
plot74 = ggplot(consol4, aes(x = coletafactor, y = pcviaveis, fill = anofactor)) + 
  geom_boxplot() + 
  scale_x_discrete(drop = FALSE) + 
  # Crucial: Keeps empty fill levels in the legend
  scale_fill_viridis_d(option = "D", drop = FALSE) + 
