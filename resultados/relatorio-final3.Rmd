...
```{r}
library(readxl)
consolidado <- read_excel('~/rstudio-projects/pesq-melinis-2024/dados/consolidado-correto.xlsx')
```
...
consolidtudo2 <- cons8
#consolidtudo2 e o geral com as viaveis por m2
```
...
```{r}
consol34 <- subset(consolidtudo2, Ambiente == 3 | Ambiente == 4)
consol34
consol12 <- subset(consolidtudo2, Ambiente == 1 | Ambiente == 2)
consol12
```
...
FAZER POISSON TREATMENTS 1 2: FERTILE SEEDS per M2 ~ ambfactor + anofactor
SEE IF SEED RAIN DISPERSES AWAY FROM PLANT AND CHECK BETWEEN YEAR VARIATION
...
FAZER BINOMIAL REGRESSION TREATS 1 2 TO CHECK PROPORTION OF FERTILE SEEDS BETWEEN YEARS AND TIME OF YEAR (AND TREATMENT?)
